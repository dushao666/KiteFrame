-- =============================================
-- V0001 初始化 RBAC 权限管理系统表结构
-- 描述: 创建用户/角色/部门/菜单等核心表与监控日志表，并初始化基础数据
-- 注意: 本脚本由 DbUp 在应用启动时自动执行，执行记录写入版本表 sys_schema_versions；
--       所有语句必须幂等（CREATE TABLE IF NOT EXISTS / INSERT IGNORE），禁止手工修改已执行的脚本
-- =============================================
...
INSERT IGNORE INTO `sys_role_menu` (`RoleId`, `MenuId`, `CreateUserId`) VALUES
-- 超级管理员拥有所有菜单权限
